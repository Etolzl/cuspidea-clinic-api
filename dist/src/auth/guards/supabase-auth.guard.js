...
let SupabaseAuthGuard = class SupabaseAuthGuard {
    ...
    async canActivate(context) {
        ...
        if (!authHeader) {
            throw new common_1.UnauthorizedException('No se proporcionó la cabecera de autorización');
        }
        ...
        if (error || !user) {
            throw new common_1.UnauthorizedException(error?.message || 'Token de autenticación inválido o expirado');
        }
        request.user = {
            id: user.id,
            email: user.email,
